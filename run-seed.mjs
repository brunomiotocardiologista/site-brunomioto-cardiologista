...
const DATABASE_URL = "mysql://root:R0bOyePHDJPJBMCyabsRniaFqCFextyq@zephyr.proxy.rlwy.net:25189/railway";
...
Se você quer entender seu perfil lipídico e saber se seu tratamento está adequado, agende uma consulta com o Dr. Bruno Mioto, **cardiologista em São Paulo** especializado em prevenção cardiovascular e dislipidemia.`,
  },
  {
    ...
    categoryId: 1,
    coverImage: "https://images.unsplash.com/photo-1530026405186-ed1f139313f8?w=1200&q=80",
...
O Dr. Bruno Mioto inclui o tratamento da obesidade como parte integral do cuidado cardiovascular em São Paulo, reconhecendo que controlar o peso é tão importante quanto controlar a pressão ou o colesterol.`,
  },
  {
    ...
    categoryId: 1,
    coverImage: "https://images.unsplash.com/photo-1628348068343-c6a848d2b6dd?w=1200&q=80",
...
async function seed() {
  ...
  for (const post of POSTS) {
    ...
    await conn.execute(
      `INSERT INTO blog_posts (id, title, slug, excerpt, content, coverImage, categoryId, published, publishedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, 1, NOW())`,
      [post.id, post.title, post.slug, post.excerpt, post.content, post.coverImage || null, post.categoryId]
    );
